...
class Humanoid(BaseTask):
    ...
    def __init__(self, cfg, sim_params, physics_engine, graphics_device, device):
        ...
        self.angular_velocity_scale = self.cfg["env"]["angularVelocityScale"]
        ...
        self.use_dof_force_sensors = self.cfg["env"]["useDofForceSensors"]

        # 108 - with dof forces observations, 87 - standard
        num_obs = 87
        if self.use_dof_force_sensors:
            num_obs = 108
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        start_pose.p = gymapi.Vec3(*get_axis_params(1.32, self.up_axis_idx))
    ...
    def compute_observations(self):
        ...
        if self.use_dof_force_sensors:
            ...
            self.obs_buf[:], self.potentials[:], self.prev_potentials[:], self.up_vec[:], self.heading_vec[:] = compute_humanoid_observations_dof_forces(
...
def compute_humanoid_observations(obs_buf, root_states, targets, potentials, inv_start_rot, dof_pos, dof_vel,
    ...
    # type: (Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, float, Tensor, Tensor, float, float, float, Tensor, Tensor) -> Tuple[Tensor, Tensor, Tensor, Tensor, Tensor]
    ...
    # obs_buf shapes: 1, 3, 3, 1, 1, 1, 1, 1, num_dofs (21), num_dofs (21), 12, num_acts (21)
    obs = torch.cat((torso_position[:, 2].view(-1, 1), vel_loc, angvel_loc * angular_velocity_scale,
                     yaw, roll, angle_to_target, up_proj.unsqueeze(-1), heading_proj.unsqueeze(-1), dof_pos_scaled,
...
@torch.jit.script
def compute_humanoid_observations_dof_forces(obs_buf, root_states, targets, potentials, inv_start_rot, dof_pos, dof_vel,
    ...
    torso_position = root_states[:, 0:3]
    torso_rotation = torch.roll(root_states[:, 3:7], 1, -1)
    velocity = root_states[:, 7:10]
    ang_velocity = root_states[:, 10:13]
