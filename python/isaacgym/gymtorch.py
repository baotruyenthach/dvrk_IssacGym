...
def _import_gymtorch():
    ...
    if torch.cuda.device_count() > 0:
        ...
        # Force torch to create a primary CUDA context on each device,
        # which will be shared by Gym physics etc.
        for i in range(torch.cuda.device_count()):
            _create_context(torch.device('cuda', i))
...
def wrap_tensor(gym_tensor, offsets=None, counts=None):
    ...
    if counts is None:
        counts = shape
    # print(dtype, data, shape, offsets, counts)
...
def unwrap_tensor(torch_tensor):
    ...
    # print(type(torch_tensor.shape))
    # print(torch_tensor.dtype)
    # print(torch_tensor.data_ptr())
