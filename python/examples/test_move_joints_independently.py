...
if sim_type is gymapi.SIM_FLEX:
    ...
    sim_params.flex.relaxation = 0.05
...
soft_asset_file = "deformable_object_grasping/examples/rectangle/soft_body.urdf"

soft_pose = gymapi.Transform()
soft_pose.p = gymapi.Vec3(0, 0.4, 0.1)
...
while not gym.query_viewer_has_closed(viewer):
    ...
    # pos_targets = upper_limits
    # pos_targets = np.zeros(num_dofs, dtype=np.float32) 
    
    # pos_targets[0] = 1.5994
    # pos_targets[1] = 0.94249
    # pos_targets[2] = 0.24
    # gym.set_actor_dof_position_targets(envs[0], kuka_handles[0], pos_targets) 
    # move_daVinci_end_effector(0.0, 0.5, 0.1, gymapi.Quat(0,0,0,1),0)
    # move_gripper(0)
    ...
    gym.set_joint_target_position(envs[0], gym.get_joint_handle(envs[0], "kuka", "psm_main_insertion_joint"), 0.19)
    ...
    gym.set_joint_target_position(envs[0], gym.get_joint_handle(envs[0], "kuka", "psm_tool_gripper1_joint"), 0.0)
