...
class GraspDataCollectionClient:
    def __init__(self):
        ...
        hand_joint_states_topic = '/reflex/joint_states'
        ...
        self.save_grasp_snap = rospy.get_param('~save_grasp_snap', True)
    ...
    def set_up_place_range(self):
        ...
        self.place_y_min = self.table_y - place_y_half_range + 0.2
    ...
    def gen_grasp_preshape_client(self):
        '''
        Generate preshape, given the target object (self.object_segment_response)
        '''
    ...
    def control_reflex_config_client(self, go_home=False, close_hand=False, grasp_preshape_idx=-1):
        '''
        close hand (grasp), go home, and set preshape
        '''
        # bao123
        rospy.loginfo('Waiting for reflex services: open_hand, grasp, set_prescontrol_reflex_config_clienthand', Trigger)
                srv_req = TriggerRequest()
            elif close_hand:
                rospy.wait_for_service('/reflex_grasp_interface/grasp')
                srv_call = rospy.ServiceProxy('/reflex_grasp_interface/grasp', Trigger)
                srv_req = TriggerRequest()
            else:
                rospy.wait_for_service('/reflex_grasp_interface/set_preshapeset_preshape')
                srv_call = rospy.ServiceProxy('/reflex_grasp_interface/set_preshape', ReflexSetPreshape)
                srv_req = ReflexSetPreshapeRequest()
                srv_req.preshape = self.preshape_response.allegro_joint_state[grasp_preshape_idx].position[0]
            srv_call(srv_req)
        except rospy.ServiceException, e:
            rospy.loginfo('Service call to reflex_grasp_interface failed: %s'%e)
        rospy.loginfo('Service to control reflex is executed')
